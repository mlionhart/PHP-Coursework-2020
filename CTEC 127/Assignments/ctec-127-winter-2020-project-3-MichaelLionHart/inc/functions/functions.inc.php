...
function display_letter_filters($filter){  
    ...
    echo '<a class="text-secondary p-2 mr-2 bg-primary text-light border rounded" href="?clearfilter" title="Reset Filter">Reset</a>&nbsp;&nbsp;';
}
...
function display_record_table($result){
    ...
    while ($row = $result->fetch_assoc()){
        ...
        echo "<td>{$row['gpa']}</td>";
        echo "<td class=\"text-center\">{$row['financial_aid']}</td>";
